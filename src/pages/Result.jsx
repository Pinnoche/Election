        ...
        <div className='py-10 md:py-16 lg:py-24 text-white md:min-h-[90vh]'>
            ...
            <div className='resultmain w-[90%] mx-auto bg-[#1D1D1D]/90 px-8 py-6 rounded-md sm:w-[80%] md:w-[70%] lg:w-[77%] xl:w-[65%] sm:py-12'>
                ...
                <div className='lg:flex flex flex-wrap items-center justify-center'>
                    {results.map((result) => (
                        <Cards onClick={()=> handleresult(result.id)} key={result.id} election_name={result.election_name} win_percent={result.win_percent}/>
                    ))}
                </div>
            </div>
            {detail && <ResultDetailCard end_date={detail.end_date} start_date={detail.start_date} ename={detail.ename}
            total_vote={detail.total_vote} mwin_percent={detail.mwin_percent} candidates={detail.candidates}/>}
        </div>
    );
}
